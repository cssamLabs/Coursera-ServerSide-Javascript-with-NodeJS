...
const getEmployeeOverFifty = (employeeData) => new Promise((resolve, reject) => {
  ...
  let employees = employeeData.filter(employee => employee.employeeAge > 50)
...
const getTotalNoOfDaysSinceJoining = (employeeData) => new Promise((resolve, reject) => {
  ...
  let days = employeeData.map(emp => Math.floor((new Date().getTime() - new Date(emp.dateOfJoining).getTime()) / (1000 * 3600 * 24)))
  resolve(days)
}
