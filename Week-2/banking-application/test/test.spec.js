...
describe('Test updating customer', () => {
    ...
    it('should update name', async () => {
        expect(customerList[0].Name).to.be.equal("Nina Sen");
        updateCustomer("c001","Nina Dobrev",24,"Kolkata,West Bengal",910855748,category("EL"));
        expect(customerList[0].Name).to.be.equal("Nina Dobrev");
    })

    it('should update phone number', async () => {
        expect(customerList[1].ContactNumber).to.be.equal(9154879626);
        updateCustomer("c002","Joey Tribbiani",30,"Boston,USA",9154879600,category("HL"));
        expect(customerList[1].ContactNumber).to.be.equal(9154879600);
    })

    it('should update age', async () => {
        expect(customerList[2].Age).to.be.equal(24);
        updateCustomer("c003","Ross Geller",34,"New York,USA",8468555748,category("VL"));
        expect(customerList[2].Age).to.be.equal(34);
    })
})
